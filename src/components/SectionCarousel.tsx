...
const SwiperButtonNext = forwardRef(function SwiperButtonNext(
  { children }: any,
  ref,
) {
...
const SwiperButtonPrev = forwardRef(function ({ children }: any, ref) {
  const swiper = useSwiper();
  const className = `${
    swiper.activeIndex == 0 ? 'opacity-30 pointer-events-none' : ''
  }`;
  return (
    <button
      className="mr-11"
...
function SectionCarousel() {
  ...
  return (
    <section
      id="carousel"
      className="max-w-[1070px] mx-auto relative mt-24 mb-32"
    >
      <div>
        <SectionTitle
        ...
        >
          <div className="absolute right-0 top-[25px]">
            ...
            <SwiperButtonNext ref={prevBtn}>
              <StaticImage src="../../static/images/next.png" alt="next" />
            </SwiperButtonNext>
          </div>
          {nodes.map(({ image, name, price }: IRecipeCard, index) => (
            <SwiperSlide key={index}>
              <RecipeCard image={image} name={name} price={price} />
            </SwiperSlide>
          ))}
        </Swiper>
      </div>
