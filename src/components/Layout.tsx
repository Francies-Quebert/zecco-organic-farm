...
export default function Layout({ children }: LayoutProps) {
    ...
    `);
    console.log(allContentfulNavBar.nodes, 'allContentfulNavBar')

    return (
        <div className="">
            <header className="bg-white flex items-center h-[80px] border border-transparent relative z-50">
                <div className='w-[40%] mx-auto'><Logo /></div>
                <div className='flex w-[60%] h-full'>
                    ...
                    <ul className='flex items-center font-semibold space-x-[92px] mr-32'>
                        {allContentfulNavBar.nodes.reverse().map(({ name, childrenContentfulNavBarContentfulchildrenJsonNode, slug }: INavBar, index) => {
                            if (childrenContentfulNavBarContentfulchildrenJsonNode.length > 0) {
                                return (<li className='group  menu-btn' key={index}>
                                    ...
                                    <ul className="bg-white absolute h-0 opacity-0 overflow-hidden group-hover:h-auto group-hover:opacity-100 shadow-lg w-44 pl-6 top-[79px] transition-opacity delay-75 ease-in-out left-[-37px]">
                                        {childrenContentfulNavBarContentfulchildrenJsonNode.length > 0
                                            && childrenContentfulNavBarContentfulchildrenJsonNode.map(({ name, slug }, i) => <li className='h-8' key={i}><Link to={slug} >{name}</Link></li>)}
                                    </ul>
                                </li>)
                            } else {
                                return <li className='menu-btn' key={index}><Link to={slug} >{name}</Link></li>
                            }
                        })}
                    ...
                    <div className='space-x-11 items-center flex'>
                        ...
                        <button className='border border-accent h-[35px] w-[124px] font-thin text-[10px]'>CREATE ACCOUNT</button>
                    </div>
                </div>
            </header>
            <div>{children}</div>
